      ...
      <div className="fixed top-24 right-5 z-50 flex flex-col gap-3">
        {toasts.map((toast) => (
          <div
            ...
            className={`
              flex items-center w-full max-w-xs p-4 mb-4 text-gray-500 bg-white rounded-lg shadow-xl cursor-pointer 
              transform transition-all duration-300 hover:scale-102 animate-slideIn
              border-l-4
              ${toast.type === 'success' ? 'border-success' : 
                toast.type === 'error' ? 'border-error' : 
                toast.type === 'warning' ? 'border-warning' : 'border-blue-500'}
            `}
            role="alert"
          >
            <div className={`
              ...
              ${toast.type === 'success' ? 'bg-green-100 text-success' : 
                toast.type === 'error' ? 'bg-red-100 text-error' : 
                toast.type === 'warning' ? 'bg-orange-100 text-warning' : 'bg-blue-100 text-blue-500'}
            `}>
              {toast.type === 'success' && (
